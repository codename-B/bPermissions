...
public class IpLock {
	...
	public IpLock(Permissions p) {
		...
		p.getServer().getPluginManager().addPermission(new Permission("bPermissions.iplock.lock", PermissionDefault.OP));
		this.p = p;
	...
	public void addIp(Player player) {
		...
		List<String> ips = c.getStringList("data."+player.getName()+".ips", new ArrayList<String>());
