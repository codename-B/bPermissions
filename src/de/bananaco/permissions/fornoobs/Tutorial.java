...
public class Tutorial extends TutorialListener {
	...
	public void stageOne(PlayerCommandPreprocessEvent event) {
		...
		player.sendMessage(ChatColor.WHITE+"/permissions "+plugin.globalCommand+" "+plugin.addGroup+" admin "+player.getName());
		this.incrementStage(player);
	}
	
	public void stageTwo(PlayerCommandPreprocessEvent event) {
		Player player = event.getPlayer();
		if(event.getMessage().toLowerCase().endsWith((plugin.globalCommand+" "+plugin.addGroup+" admin "+player.getName()).toLowerCase() )) {
		player.sendMessage(ChatColor.GREEN+"--"+ChatColor.BLUE+"bPermissions tutorial"+ChatColor.GREEN+"--");
		player.sendMessage(ChatColor.BLUE+"To add a permission node to a group, try it!");
		player.sendMessage(ChatColor.WHITE+"/permissions "+plugin.globalCommand+" "+plugin.addNode+" bPermissions.admin admin");
		this.incrementStage(player);
		} else {
	...
	public void stageThree(PlayerCommandPreprocessEvent event) {
		Player player = event.getPlayer();
		if(event.getMessage().toLowerCase().endsWith( (plugin.globalCommand+" "+plugin.addNode+" bPermissions.admin admin").toLowerCase() ) ) {
		player.sendMessage(ChatColor.GREEN+"--"+ChatColor.BLUE+"bPermissions tutorial"+ChatColor.GREEN+"--");
		player.sendMessage(ChatColor.BLUE+"If you wanted to remove a permission node from a group for one world only, try it!");
		player.sendMessage(ChatColor.WHITE+"/permissions "+plugin.worldCommand+" " +plugin.removeNode+" bPermissions.build default "+player.getWorld().getName());
		this.incrementStage(player);
		} else {
		player.sendMessage(ChatColor.RED+"Try again!");
		this.decrementStage(player);
	...
	public void stageFour(PlayerCommandPreprocessEvent event) {
		Player player = event.getPlayer();
		if(event.getMessage().toLowerCase().endsWith((plugin.worldCommand+" " +plugin.removeNode+" bpermissions.build default "+player.getWorld().getName()).toLowerCase())) {
		player.sendMessage(ChatColor.GREEN+"--"+ChatColor.BLUE+"bPermissions tutorial"+ChatColor.GREEN+"--");
