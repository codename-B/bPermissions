...
public class Tracks {
	...
	public Tracks(Permissions p) {
		...
		for (String permission : getPermissions()) {
			p.getServer()
					.getPluginManager()
					.addPermission(
							new Permission(permission, PermissionDefault.OP));
		}
	}
	...
	private void setupDefaults() {
		...
		addTrack("admin", Arrays.asList("moderator", "admin"),
				Arrays.asList(worlds), "You are now an admin boi!",
				"Not so fast :(");
		addTrack("moderator", Arrays.asList("moderator"),
				Arrays.asList(worlds), "You have been promoted to moderator!",
				"Bad boy!");
	}
	...
	public boolean promote(String player, String track) {
		...
		if (!tracks.contains(track))
			return false;
		// The groups
		List<String> groups = c.getStringList("tracks." + track + ".groups",
				new ArrayList<String>());
		List<String> worldStrings = c.getStringList("tracks." + track
		...
		// The message
		String message = c.getString("tracks." + track + ".promote-message",
				defaultPromoteMessage);
		// Sending the message
	...
	public boolean demote(String player, String track) {
		...
		for (String world : worldStrings)
			if (p.getServer().getWorld(world) != null)
				worlds.add(p.getServer().getWorld(world));
		// The message
		String message = c.getString("tracks." + track + ".demote-message",
				defaultDemoteMessage);
		// Sending the message
		Player pl = p.getServer().getPlayer(player);
		if (pl != null) {
	...
	public void addTrack(String trackname, List<String> groups,
			List<String> worlds, String promoteMessage, String demoteMessage) {
		c.setProperty("tracks." + trackname + ".groups", groups);
		c.setProperty("tracks." + trackname + ".worlds", worlds);
		c.setProperty("tracks." + trackname + ".promote-message",
				promoteMessage);
