...
public class PermissionsExec {
	...
	public boolean exec(CommandSender sender, String[] args, String world) {
		...
		else if(args.length >= 4 && args[1].equalsIgnoreCase(plugin.removeGroup)) {
		String player = args[2];
		String group = args[3];
		
		PermissionSet p = plugin.pm.getPermissionSet(w);
		p.removeGroup(player, group);
		sender.sendMessage("Removed group:"+group+" from player:"+player);
		return true;
		}
		else if(args.length >= 3 && args[1].equalsIgnoreCase(plugin.listGroup)) {
		String player = args[2];
			
		PermissionSet p = plugin.pm.getPermissionSet(w);
		List<String> playerGroups = p.getGroups(player);
		String list = Arrays.toString(playerGroups.toArray());
		sender.sendMessage(player+" in world:"+world+" has these groups:");
		...
		sender.sendMessage("Added node:"+node+" to group:"+group+" in world:"+world);
		return true;
		}
		else if(args.length >= 4 && args[1].equalsIgnoreCase(plugin.removeNode)) {
		String node = args[2];
		String group = args[3];
		
		PermissionSet p = plugin.pm.getPermissionSet(w);
		p.removeNode(node, group);
		sender.sendMessage("Removed node:"+node+" from group:"+group+" in world:"+world);
		return true;
		}
		else if(args.length >= 3 && args[1].equalsIgnoreCase(plugin.listNode)) {
		String group = args[2];

		PermissionSet p = plugin.pm.getPermissionSet(w);
		List<String> groupNodes = p.getGroupNodes(group);
		String list = Arrays.toString(groupNodes.toArray());
		sender.sendMessage(group+" in world:"+world+" has these nodes:");
		sender.sendMessage(list);
		return true;
		}
		return false;
